...
/*   Updated: 2019/06/12 14:14:29 by Roger Ndaba      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Form::beSigned(Bureaucrat const& bureaucrat) {
    if (bureaucrat.getGrade() <= _toSign && bureaucrat.getGrade() <= _toExcecute) {
        std::cout << bureaucrat.getName() << " signs " << _name << std::endl;
        this->_signed = true;
    } else {
        std::cout << bureaucrat.getName() << " cannot sign " << _name << " because grade is too low" << std::endl;
    }
}
